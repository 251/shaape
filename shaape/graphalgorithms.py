...
def reduce_path(nodes):
    ...
    for i in range(2, len(nodes)):
        ...
        if new_nodes[-1].fusable() and has_same_direction(previous_edge, current_edge):
            new_nodes[-1] = nodes[i]
        else:
    ...
    if len(new_nodes) > 2 and new_nodes[0] == new_nodes[-1]:
        ...
        if new_nodes[-1].fusable() and has_same_direction(first_edge, last_edge):
            del new_nodes[-1]
    ...
    for i in range(1, len(_cycle) - 2):
        ...
        for n in neighbors:
            if n != _cycle[i + 1] and n != _cycle[i - 1]:
                ...
                if angle_to_neighbor > angle_to_next_node and cycle_is_ccw:
                    return False
                if angle_to_neighbor < angle_to_next_node and not cycle_is_ccw:
                    return False
    return True
...
def line_segments_intersect(seg1, seg2):
    return ccw(seg1[0], seg2[0], seg2[1]) != ccw(seg1[1], seg2[0], seg2[1]) and ccw(seg1[0], seg1[1], seg2[0]) != ccw(
        seg1[0], seg1[1], seg2[1])
...
def line_segments_distance(seg1, seg2):
    if line_segments_intersect(seg1, seg2):
        ...
        distances = [line_segment_point_distance(seg1[0], seg2), line_segment_point_distance(seg1[1], seg2),
                     line_segment_point_distance(seg2[0], seg1), line_segment_point_distance(seg2[1], seg1)]
