            ...
            if polygons_containing_this_text:
                ...
                for graph in graphs:
                    for edge in graph.edges():
                        if line_segments_distance((edge[0].position(), edge[1].position()), (
                                text.letter_position(0), text.letter_position(len(text.text()) - 1))) <= 1:
                            graph.add_name(text.text())
                for arrow in arrows:
                    if line_segment_point_distance(arrow.tip(), (
                            text.letter_position(0), text.letter_position(len(text.text()) - 1))) <= 1:
